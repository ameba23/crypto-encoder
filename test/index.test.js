...
const streamXORInstanceEncoder = require('../stream-xor-instance')
...
describe('crypto_stream_XOR_instance', (context) => {
  let storage
  ...
  context('encrypt and decrypt a message', (assert, next) => {
    const key = streamXORInstanceEncoder.encryptionKey()
    const encoder1 = streamXORInstanceEncoder(key, { valueEncoding: 'utf-8' })
    const encoder2 = streamXORInstanceEncoder(key, { valueEncoding: 'utf-8', nonce: encoder1.nonce })
  ...
  context('hypercore - encrypt the log - binary encoding', (assert, next) => {
    const key = streamXORInstanceEncoder.encryptionKey()
    const encoder = streamXORInstanceEncoder(key)
  ...
  context('hypercore - encrypt the log - utf-8 encoding', (assert, next) => {
    const key = streamXORInstanceEncoder.encryptionKey()
    const encoder = streamXORInstanceEncoder(key, { valueEncoding: 'utf-8' })
    const testEncoder = streamXORInstanceEncoder(key, { valueEncoding: 'utf-8', nonce: encoder.nonce })
    const feed = hypercore(storage, { valueEncoding: encoder })
  ...
  context('hypercore - encrypt the log - json encoding', (assert, next) => {
    const key = streamXORInstanceEncoder.encryptionKey()
    const encoder = streamXORInstanceEncoder(key, { valueEncoding: 'json' })
    const testEncoder = streamXORInstanceEncoder(key, { valueEncoding: 'json', nonce: encoder.nonce })
    const feed = hypercore(storage, { valueEncoding: encoder })
